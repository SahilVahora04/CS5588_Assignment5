...
func main() {
	...
	for _, duration := range durations {
		...
		for _, entry := range urls {
			...
			// metrics per second
			apiCallsPerSecond.WithLabelValues(entry.Query).Set(apiCalls.WithLabelValues(entry.Query).Value() / duration.Seconds())
			dataCollectedPerSecond.WithLabelValues(entry.Query).Set(dataCollected.WithLabelValues(entry.Query).Value() / duration.Seconds())
		}
